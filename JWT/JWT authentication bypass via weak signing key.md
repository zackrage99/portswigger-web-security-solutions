...
7. **Attack**: First go to `/jwt.secrets.list` on github provided by port swigger download the whole list
...
```
10. hashcat immedialty cracked the password which is 'secret1'
11. The next step is to encode the `secret1` as base64 from burp go to Decoder tab paste your text encode as base64 now copy the result
12. From the JWTeditor tab go to New Symmetric Key and click generate and paste your base64 text in k parameter
13. Now after we hav our key we need to sign it
14. Go to repeater change the request `GET /my-account?id=wiener` to `GET /admin` and from jsonwebtoken tab change `"sub": "wiener"` to `"sub": "administrator"` and send the request
15. Notice You got 200 ok response meaning now are now in the administrator panel search for `carlos` you will find a link to delete the user carlos copy it and replace it in GET /admin
16. Lab solved
